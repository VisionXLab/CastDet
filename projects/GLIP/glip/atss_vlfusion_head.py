...
class RotatedVLFusionModule(BaseModel):
    """Rotated Visual-lang Fusion Module."""
    ...
    def _init_layers(self) -> None:
        ...
        self.bbox_pred = nn.Conv2d(
            self.feat_channels, self.num_base_priors * self.reg_dim, kernel_size=1)
        ...
        self.scales = nn.ModuleList([Scale(1.0) for _ in range(self.num_levels)])
...
class RotatedATSSVLFusionHead(RotatedATSSHead):
    """Rotated ATSS head with visual-language fusion module.
    ...
    def __init__(self,
        ...
        self.head = RotatedVLFusionModule(
        ...
        for bbox_pred, centerness in zip(bbox_preds, centernesses):
            ...
            bbox_preds_.append(
                bbox_pred.permute(0, 2, 3,
                                  1).reshape(cls_scores.size(0), -1, dim))
    ...
    def _loss_by_feat(self, anchors: Tensor, cls_score: Tensor,
        ...
        bbox_pred = bbox_pred.reshape(-1, dim)
        ...
        if pos_inds.sum() > 0:
            ...
            # Rotated GLIP ignores this setting.
            # beacause the rotated angle representation is xywha rather than xyxy
    ...
    def _get_targets_single(self,
        ...
        """
        # the original GLIP set the `inside_flags=True`, which is different from ATSS
        anchors = flat_anchors
        inside_flags = torch.ones(len(anchors), dtype=bool,
